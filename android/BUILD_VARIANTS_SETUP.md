...
| Feature | Dev | Prod |
|---------|-----|------|
| Package Name | `com.viewridgesecurity.smsgateway.dev` | `com.viewridgesecurity.smsgateway` |
| App Name | "SMS Gateway (Dev)" | "SMS Gateway" |
| API Base URL | `https://api.dev.textbee.dev/api/v1/` | `https://sms.viewridgesecurity.com/api/v1/` |
...
#### For Production:
- In Firebase project `VRS-SMS` (`vrs-sms`), register an Android app whose package name is exactly `com.viewridgesecurity.smsgateway`
- Download the generated file as `app/src/prod/google-services.json`
- Do not reuse or hand-edit the configuration generated for the legacy upstream package; the Firebase client registration must match the Gradle `applicationId`

#### For Development:
- Create a new Firebase project for development
- Download the `google-services.json` for your dev project
- **Important**: Make sure the package name in Firebase is set to `com.viewridgesecurity.smsgateway.dev`
...
- **Prod**: `https://sms.viewridgesecurity.com/api/v1/`
...
### Firebase Issues:
- Confirm the package name in Firebase matches the variant (`com.viewridgesecurity.smsgateway.dev` for dev)
